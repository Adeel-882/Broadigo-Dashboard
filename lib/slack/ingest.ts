...
async function processStoredMessage(db:Database,channel:Channel,message:Message){
  ...
  const base={slackMessageId:message.id,employeeId:attribution.employeeId,teamId:channel.teamId,occurredAt:message.postedAt,confidence:String(result.confidence)};const value=(key:string)=>result.values[key] as string|null|undefined;
  if(result.recordType==="APPOINTMENT")await db.insert(appointments).values({...base,prospectName:value("prospectName"),phone:value("phone"),state:value("state"),originalTimezone:value("originalTimezone"),assignedPerson:value("assignedPerson")}).onConflictDoUpdate({target:appointments.slackMessageId,set:{employeeId:base.employeeId,teamId:base.teamId,occurredAt:base.occurredAt,confidence:base.confidence,prospectName:value("prospectName"),phone:value("phone"),state:value("state"),originalTimezone:value("originalTimezone"),assignedPerson:value("assignedPerson")}});
...
export async function ingestSlackHistoryBatch(workspaceId:string,slackChannelId:string,history:SlackHistoryMessage[]){
  ...
  return db.transaction(async tx=>{
    ...
    for(const item of usable){const old=existingByTs.get(item.ts);if(old&&reactionsFingerprint(old.reactions)!==reactionsFingerprint(item.reactions))reactionRefresh.push({id:old.id,reactions:item.reactions??[]});if(old&&old.parserStatus!=="PENDING"&&old.parserStatus!=="ERROR")continue;const messageId=old?.id??randomUUID();const employeeId=old?.employeeId??(item.user?identityByUser.get(item.user):undefined)??null;const postedAt=old?.postedAt??slackDate(item.ts);const result=parserRegistry.parse(channel.parserType,{rawSourceId:messageId,text:item.text!,postedAt,employeeId});
      ...
      if(result.recordType==="APPOINTMENT")appointmentsRows.push({...base,prospectName:value("prospectName"),phone:value("phone"),state:value("state"),originalTimezone:value("originalTimezone"),assignedPerson:value("assignedPerson")});
