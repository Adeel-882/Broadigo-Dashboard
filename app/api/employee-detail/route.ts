import { getEmployeeDetail } from "@/lib/dashboard-data";
...
export async function GET(request: Request) {
  ...
  try {
    return Response.json(await getEmployeeDetail(employeeId, period, url.searchParams.get("start") ?? undefined, url.searchParams.get("end") ?? undefined));
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "Unable to load employee details." }, { status: 400 });
  }
}
